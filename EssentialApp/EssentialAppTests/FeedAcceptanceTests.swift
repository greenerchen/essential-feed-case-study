...
class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onEnteringBackground_deletesExpiredFeedCache() {
        ...
        XCTAssertNil(store.feedCache, "Expected to delete expired feed cache")
    }
    ...
    func test_onEnteringBackground_keepsNonExpiredFeedCache() {
        ...
        XCTAssertNotNil(store.feedCache, "Expected to keep non-expired feed cache")
    }
    ...
    private func launch(
        ...
        let scene = SceneDelegate(httpClient: httpClient, store: store, scheduler: .immediateOnMainQueue)
        scene.window = UIWindow(frame: CGRect(x: 0, y: 0, width: 390, height: 1))
        scene.configureWindow()
        let nav = scene.window?.rootViewController as? UINavigationController
        let sut = nav?.topViewController as! ListViewController
    ...
    private func makeSecondFeedPageData() -> Data {
        try! JSONSerialization.data(withJSONObject: [
            "items": [
                ["id": "166FCDD7-C9F4-420A-B2D6-CE2EAFA3D82F", "image": "http://image.com/image-2"]
            ]
        ])
    }
        ...
        try! JSONSerialization.data(withJSONObject: [
            "items": [
                [
                    ...
                    "created_at": "2023-12-25T13:45:30+00:00",
                    "author": [
                        "username": "a username"
                    ]
                ] as [String: Any]
